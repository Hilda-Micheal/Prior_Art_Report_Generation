...
def build_query(keywords, cpc, ipc):
    ...
    for code in cpc:
        # Correct Lens.org query field: class_cpc.symbol
        # Escape slash: G06N3/08 -> G06N3\/08
...
def build_request_body(query, offset, size, year_from=None, year_to=None):
    body = {
        "query": query,
        "size": size,
        "from": offset,
        "sort": [{"date_published": "desc"}],
        # Explicitly request fields — "biblio" covers title, abstract, IPC, CPC
        # "reference_cited" is the top-level field for backward citations (what this patent cites)
        # No include filter — get all available fields including reference_cited when present
    }
...
# API caller
# ---------------------------------------------------------------------------
def fetch_patents(api_key, query, total_limit, year_from=None, year_to=None):
    headers = {
    ...
    while len(all_records) < total_limit:
        ...
        body = build_request_body(query, offset, batch_size, year_from=year_from, year_to=year_to)
...
# ---------------------------------------------------------------------------
# Parser — field paths confirmed from debug output:
#
#   Title    : biblio.invention_title[0].text
#   Abstract : abstract[0].text
#   IPC      : biblio.classifications_ipcr.classifications[].symbol
#   CPC      : biblio.classifications_cpc.classifications[].symbol
#   Citations: biblio.cited_by (dict — count keys if present)
...
def parse_patent(raw):
    ...
    # --- IPC codes ---
    # Path: biblio.classifications_ipcr.classifications[].symbol
    ...
    # --- CPC codes ---
    # Path: biblio.classifications_cpc.classifications[].symbol
    ...
    # --- Citations ---
    # reference_cited  = backward citations — patents THIS patent cites (top-level field)
    # biblio.cited_by  = forward citations — patents that cite THIS patent
    #                    (often empty; Lens does not always populate forward citations per record)

    # Backward citations: top-level "reference_cited" field
    ref_cited = raw.get("reference_cited", {})
    if isinstance(ref_cited, dict):
        cites_patents = ref_cited.get("patent_count", 0) or 0
        cites_npl     = ref_cited.get("npl_count", 0) or 0
        cites_total   = cites_patents + cites_npl
    else:
        cites_total = 0

    # Forward citations: biblio.cited_by (populated only for some records)
    cited_by = biblio.get("cited_by", {})
    if isinstance(cited_by, dict):
        cited_by_patents = cited_by.get("patent_count", 0) or 0
        cited_by_npl     = cited_by.get("npl_count", 0) or 0
        cited_by_total   = cited_by_patents + cited_by_npl
    else:
        cited_by_total = 0

    citations = f"cited_by:{cited_by_total} | cites:{cites_total}"
...
def main():
    ...
    raw_records = fetch_patents(api_key, query, args.limit,
