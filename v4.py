...
def parse_patent(raw):
    ...
    # biblio.references_cited.citations[i].patcit.lens_id
    refs_cited = biblio.get("references_cited", {})
    cites_ids = []
    if isinstance(refs_cited, dict):
        for c in refs_cited.get("citations", []):
            patcit = c.get("patcit", {})
            cid = patcit.get("lens_id", "")
            if cid:
                cites_ids.append(cid)
    cites = "; ".join(cites_ids)

    # --- Cited by: patents that cite THIS patent (forward citations) ---
    # biblio.cited_by.patents[i].lens_id
