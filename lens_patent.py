...
def build_sort(has_keywords):
    ...
    - Keywords present  → sort by relevance score first, then recency as tiebreaker.
                          This surfaces the TOP MATCHES for the given abstract/keywords.
    - CPC/IPC only      → sort by recency only (relevance score is not meaningful
                          for exact classification code lookups — all matches score equally).
    """
    if has_keywords:
        # Relevance-first: best semantic/keyword match at the top
        # Recency as tiebreaker among equally scored results
        return [
            {"_score": "desc"},
            {"date_published": "desc"}
        ]
    else:
        # Classification-code-only search: newest patents first
...
def parse_patent(raw):
    ...
    # Forward citations: biblio.cited_by
...
def main():
    ...
    # --- Smart sort: relevance if keywords present, recency if codes only ---
